...
2. Define your [configuration YAML file](configs/visual_config.yaml) for the visual data generator.
    ...
    - Note: only modify the fields under `input` and `output`, `processing` fields can also be modified but please be aware that it may require understanding how these parameters change the visual synthesis. For the most part, the comments should explain what they do.
